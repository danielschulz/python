	...
	cat >&2 <<-'EOW'
		...
		📎 Hey there!  It looks like you're trying to run a Docker daemon.
		   ...
		   You probably should use the "dind" image variant instead, something like:
		     ...
		     docker run --privileged --name some-docker ... danielschulz/docker-in-docker:20-dind ...

		   See https://hub.docker.com/r/danielschulz/docker-in-docker/tags for more documentation and usage examples.
